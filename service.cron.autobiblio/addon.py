...
def log(msg, level=xbmc.LOGNOTICE):
	xbmc.log("["+__addon__.getAddonInfo('id')+"-"+__addon__.getAddonInfo('version')+"](addon.py) "+py2_enc(msg), level)
...
def addDir(name, url, mode, image, source=None):
	u = '{0}?url={1}&mode={2}&name={3}&source={4}'.format(sys.argv[0], quote_plus(url), str(mode), quote_plus(name), quote_plus(source))
